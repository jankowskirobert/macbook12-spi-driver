...
/*
 * Since we target kernel 6.14+, we can assume modern SPI properties API
...
{
	/*
	 * EFI variable API has changed significantly in newer kernels.
	 * The efivar_entry functions are no longer available.
	 * For kernel 6.14+ compatibility, this functionality is disabled.
	 * TODO: Migrate to new EFI runtime services API if needed.
	 */
	return -ENOENT; /* Pretend no saved level exists */
}
